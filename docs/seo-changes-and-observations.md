...
- Replaced the damaged cutout social preview with the Pencil-authored Option 1 card at `/img/social-card-pencil-2026.jpg`: a 1200 × 630 JPEG using the supplied intact portrait as a rectangular left photo panel, white canvas, navy/charcoal typography, and a restrained yellow accent.
...
- Verified the Pencil `.pen` source, JPEG dimensions and size, and visually inspected the exported card: the supplied portrait is intact, rectangular, background-retained, and shows the complete head and hairline without masking artifacts or halo; confirmed the production build plus generated homepage metadata.
...
### Product Usage Scoring & Routing — Kimi K3 review follow-up (case study, homepage, video, carousel)

- Case study now carries the honest metrics narrative implemented in the app: the synthetic comparison is explicitly illustrative (not a claim of predictive superiority), the naive queue's 100% routed yield at the default capacity of 12 is named as a small-sample artifact (all twelve routed accounts carry synthetic positive outcomes), and the gated policy is positioned on context-specific plays, hard blocks, ownership, SLAs, rejected alternatives, and explainability rather than universal predictive superiority. Metrics values are untouched.
- HubSpot proof section now truthfully records the verified test-account integration (real adapter sync of the fictional designated account on 2026-08-14 — company record with `usage_routing` properties, contact, association, one task, idempotent reruns; read-only company-record app card installed, fail-closed authenticated endpoint) and explicitly states that this is a test-account capture in text, not a public HubSpot view, and that no privacy-safe screenshot exists yet (authenticated card view requires the HubSpot app client secret, not retrievable locally). No capture was fabricated.
- Homepage now features the case study as a third proof row ("Independent demonstration", labeled Product Usage Scoring & Routing, queue screenshot) directly in the featured section under "See what I've shipped for product teams", using the existing row design; the section intro now reads "Client engagements and an independent demonstration". Genrupt and Palabruno rows are unchanged and remain first.
- Launch video re-rendered (40.667s, 1080x1080, H.264/AAC, HyperFrames check 0 errors / 0 layout issues, 17-frame contact sheet inspected): the end card now shows the readable case-study URL `hanifcarroll.com/case-studies/product-usage-scoring-routing/`, and the naive comparison frame carries a legible small-sample caveat. Served unchanged at `/videos/projects/product-usage-scoring-routing/overview.mp4` (poster and webp variants regenerated).
- Carousel source regenerated: cover now carries a small readable `hanifcarroll.com · @hanifcarroll` attribution, and the comparison slide adds the same small-sample/illustrative explanation; synthetic disclosure and final URL preserved. 7 slides (1080x1080), 7-page 1080x1080 PDF, and 4x2 contact sheet regenerated.
- Verified with `bun run check:projects`, `bun run lint`, and `bun run build` (astro check + 35 pages + visitor-facing copy check), plus rendered-frame inspection of the final MP4 (comparison caveat and end-card URL readable, no clipping or caption collisions). `bun run format:check` still fails on 31 pre-existing video-project files committed at `0e6bd968` (verified identical against the baseline archive); this session introduced no new violations (the one new file, `carousel/source/render.mjs`, is formatted).
- After deployment, verify the live homepage proof row and link, the updated case-study copy and video, and that the served overview.mp4 matches the re-rendered asset (duration and byte size).
