...
Scene 3 (4.9–7.0s): right card contrasts: a 100% routed-yield fill and a "no gates" line; a cobalt `split-highlight` band (4px left rule) underlines "deferred 13" on the left as the difference ("defers 13" ≈5.0s, "No gates" ≈6.5s). The comparison holds.
...
Scene 3 (3.8–6.357s): the two action cards resolve and hold as the still end-card; one solid cobalt `cta-button` ("Open the case study · live demo") sits centered beneath on the last cue ("decision by decision" ≈4.9–6.2s); at most **subtle jitter** (`sine-wave-loop`) keeps the rings ambient. Final frame — the only frame with a true exit hold. Progress bar reaches full.
